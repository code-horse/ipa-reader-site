---
layout: bare
title: Rozšíření IPA Reader - Uživatelská příručka
lang: cs
---

# IPA Reader - Uživatelská příručka

> Verze: v1.1.0

## Úvod

IPA Reader je rozšíření prohlížeče určené pro studenty angličtiny. Přidává IPA (Mezinárodní fonetická abeceda) anotace výslovnosti k anglickým slovům na webových stránkách, podporuje americký i britský anglický přízvuk a pomáhá vám snadněji se učit anglickou výslovnost.

---

## Hlavní funkce

- **Anotace výběrem textu** — Vyberte anglický text na webových stránkách pro automatické zobrazení IPA a tlačítek pro přehrání
- **Režim IPA celé stránky** — Přidejte IPA anotace ke všem anglickým slovům na stránce jedním kliknutím
- **Americký a britský přízvuk** — Přepínejte mezi americkou angličtinou (en-US) a britskou angličtinou (en-GB) IPA
- **Text-to-Speech** — Klikněte na tlačítko reproduktoru pro přehrání výslovnosti podle zvoleného přízvuku
- **Přehrání výběru** — Vyberte libovolný anglický text, klikněte na plovoucí tlačítko nebo pravým tlačítkem zvolte „Speak Selection“ pro hlasité předčítání
- **Tipy při najetí kurzorem** — Najetím kurzoru nad anotovaná slova zobrazíte IPA a tlačítka pro výslovnost
- **Vícejazyčné rozhraní** — Podporuje 38 jazyků rozhraní

---

## Jak používat

### Krok 1: Nainstalovat rozšíření

Nainstalujte **IPA Reader** z [Chrome Web Store](https://chromewebstore.google.com/) nebo jej načtěte lokálně v režimu vývojáře.

### Krok 2: Otevřít libovolnou webovou stránku

Navštivte libovolnou webovou stránku obsahující anglický obsah.

### Krok 3: Vybrat text nebo použít plovoucí tlačítko

Vyberte anglický text, který chcete anotovat, nebo klikněte na plovoucí tlačítko v pravém dolním rohu pro zapnutí režimu IPA celé stránky.

### Krok 4: Zobrazit IPA

Najděte kurzorem nad slovy pro zobrazení IPA tipů, klikněte na ikonu reproduktoru pro přehrání výslovnosti.

### Krok 5: Přehrát vybraný text

Vyberte libovolný anglický text myší, klikněte na plovoucí tlačítko reproduktoru pro přehrání; nebo klikněte pravým tlačítkem a zvolte „Speak Selection“.

> **Tip:** Kliknutím na ikonu rozšíření na liště prohlížeče otevřete panel nastavení a upravte typ přízvuku, rychlost přehrávání a další.

---

## Přehrání výběru

Funkce přehrání výběru vám umožňuje vybrat libovolný anglický text a přečíst jej nahlas jedním kliknutím — ideální pro učení výslovnosti vět a čtecí praxi.

**Metoda 1: Plovoucí tlačítko**  
Vyberte anglický text myší, v pravém horním rohu výběru se zobrazí tlačítko reproduktoru — klikněte pro přehrání.

**Metoda 2: Kontextové menu**  
Po výběru anglického textu klikněte pravým tlačítkem a zvolte „Speak Selection“ z menu.

> **Tip:** Přehrání výběru používá rychlost přehrávání vět nastavenou ve vašich nastaveních. Výslovnost jednotlivých slov používá výchozí rychlost. Hlas TTS odpovídá zvolenému typu přízvuku (americký nebo britský).

---

## Průvodce nastavením

| Nastavení | Popis |
|---------|-------------|
| **Povolit IPA** | Hlavní přepínač pro zapnutí nebo vypnutí funkce IPA anotace |
| **IPA celé stránky** | Při zapnutí zobrazuje IPA pro všechna anglická slova (může ovlivnit rozložení stránky) |
| **Typ přízvuku** | Volba mezi americkou a britskou angličtinou IPA a výslovností |
| **Rychlost přehrávání vět** | Úprava rychlosti čtení vět (přehrání jednotlivých slov není ovlivněno) |
| **Tipy při najetí kurzorem** | Zobrazení IPA tipu při najetí kurzorem myši |

---

## Časté dotazy

**Q: Proč to na některých stránkách nefunguje?**  
A: Z bezpečnostních důvodů se rozšíření prohlížeče nemohou spouštět na speciálních stránkách jako `chrome://`, nastavení prohlížeče nebo Chrome Web Store.

**Q: Co když IPA chybí u některých slov?**  
A: Slovník IPA pokrývá běžná anglická slova. U některých vzácných nebo specializovaných slov nemusí být záznamy IPA. Slovník průběžně rozšiřujeme.

**Q: Žádný zvuk z text-to-speech?**  
A: Zkontrolujte nastavení hlasitosti systému a ověřte, že máte nainstalované anglické hlasové balíčky. Podpora řeči se liší v různých prohlížečích a operačních systémech.

**Q: Režim celé stránky ovlivňuje rozložení?**  
A: IPA anotace vyžadují dodatečný prostor, což může ovlivnit původní rozložení stránky. Pokud to ovlivňuje čtení, vypněte režim celé stránky a místo toho použijte tipy při najetí kurzorem.

---

## Související odkazy

- [Zásady ochrany osobních údajů](../privacy-policy)
- [Podpora a zpětná vazba](../support)

---
