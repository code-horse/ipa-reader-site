---
layout: bare
title: Tiện ích IPA Reader - Hướng dẫn sử dụng
lang: vi
---

# IPA Reader - Hướng dẫn sử dụng

> Phiên bản: v1.1.0

## Giới thiệu

IPA Reader là tiện ích mở rộng trình duyệt được thiết kế cho người học tiếng Anh. Nó thêm chú thích phát âm IPA (Bảng chữ cái phiên âm quốc tế) vào các từ tiếng Anh trên trang web, hỗ trợ cả giọng Mỹ và Anh, giúp bạn học phát âm tiếng Anh dễ dàng hơn.

---

## Tính năng chính

- **Chú thích văn bản được chọn** — Chọn văn bản tiếng Anh trên trang web để tự động hiển thị IPA và nút phát âm
- **Chế độ IPA toàn trang** — Thêm chú thích IPA cho tất cả từ tiếng Anh trên trang chỉ với một cú nhấp chuột
- **Giọng Mỹ và Anh** — Chuyển đổi giữa IPA tiếng Anh Mỹ (en-US) và tiếng Anh Anh (en-GB)
- **Chuyển văn bản thành giọng nói** — Nhấp vào nút loa để nghe phát âm phù hợp với giọng bạn đã chọn
- **Đọc văn bản được chọn** — Chọn bất kỳ văn bản tiếng Anh nào, nhấp nút nổi hoặc nhấp chuột phải "Speak Selection" để đọc to
- **Chú giải khi di chuột** — Di chuột qua từ được chú thích để xem IPA và nút phát âm
- **Giao diện đa ngôn ngữ** — Hỗ trợ 38 ngôn ngữ giao diện

---

## Cách sử dụng

### Bước 1: Cài đặt tiện ích

Cài đặt **IPA Reader** từ [Chrome Web Store](https://chromewebstore.google.com/), hoặc tải cục bộ ở chế độ nhà phát triển.

### Bước 2: Mở bất kỳ trang web nào

Truy cập bất kỳ trang web nào có nội dung tiếng Anh.

### Bước 3: Chọn văn bản hoặc sử dụng nút nổi

Chọn văn bản tiếng Anh bạn muốn chú thích, hoặc nhấp vào nút nổi ở góc dưới bên phải để bật chế độ IPA toàn trang.

### Bước 4: Xem IPA

Di chuột qua từ để xem chú giải IPA, nhấp vào biểu tượng loa để nghe phát âm.

### Bước 5: Đọc văn bản được chọn

Chọn bất kỳ văn bản tiếng Anh nào bằng chuột, nhấp nút loa nổi để đọc; hoặc nhấp chuột phải và chọn "Speak Selection".

> **Mẹo:** Nhấp vào biểu tượng tiện ích trên thanh công cụ trình duyệt để mở bảng cài đặt và điều chỉnh loại giọng, tốc độ đọc và hơn thế nữa.

---

## Đọc văn bản được chọn

Tính năng đọc văn bản được chọn cho phép bạn chọn bất kỳ văn bản tiếng Anh nào và đọc to chỉ với một cú nhấp chuột — hoàn hảo cho việc học phát âm câu và luyện đọc.

**Cách 1: Nút nổi**  
Chọn văn bản tiếng Anh bằng chuột, nút loa xuất hiện ở góc trên bên phải vùng chọn — nhấp để đọc.

**Cách 2: Menu nhấp chuột phải**  
Sau khi chọn văn bản tiếng Anh, nhấp chuột phải và chọn "Speak Selection" từ menu.

> **Mẹo:** Đọc văn bản được chọn sử dụng tốc độ đọc câu được cấu hình trong cài đặt của bạn. Phát âm từ đơn sử dụng tốc độ mặc định. Giọng TTS khớp với loại giọng bạn đã chọn (Mỹ hoặc Anh).

---

## Hướng dẫn cài đặt

| Cài đặt | Mô tả |
|---------|-------|
| **Enable IPA** | Công tắc chính để bật hoặc tắt tính năng chú thích IPA |
| **Whole Page IPA** | Khi bật, hiển thị IPA cho tất cả từ tiếng Anh (có thể ảnh hưởng đến bố cục trang) |
| **Accent Type** | Chọn giữa IPA và phát âm tiếng Anh Mỹ và Anh |
| **Sentence Speech Rate** | Điều chỉnh tốc độ đọc câu (phát âm từ đơn không bị ảnh hưởng) |
| **Hover Tooltips** | Hiển thị chú giải IPA khi di chuột qua |

---

## Câu hỏi thường gặp

**H: Tại sao không hoạt động trên một số trang?**  
Đ: Vì lý do bảo mật, tiện ích trình duyệt không thể chạy trên các trang đặc biệt như `chrome://`, cài đặt trình duyệt hoặc Chrome Web Store.

**H: Nếu thiếu IPA cho một số từ thì sao?**  
Đ: Từ điển IPA bao gồm các từ tiếng Anh phổ biến. Một số từ hiếm hoặc chuyên ngành có thể không có mục IPA. Chúng tôi không ngừng mở rộng từ điển.

**H: Không có âm thanh từ chuyển văn bản thành giọng nói?**  
Đ: Vui lòng kiểm tra cài đặt âm lượng hệ thống và đảm bảo đã cài đặt gói giọng tiếng Anh. Hỗ trợ giọng nói khác nhau tùy theo trình duyệt và hệ điều hành.

**H: Chế độ toàn trang ảnh hưởng đến bố cục?**  
Đ: Chú thích IPA cần thêm không gian, có thể ảnh hưởng đến bố cục trang gốc. Nếu ảnh hưởng đến việc đọc, hãy tắt chế độ toàn trang và dùng chú giải khi di chuột thay thế.

---

## Liên kết liên quan

- [Privacy Policy](../privacy-policy)
- [Support & Feedback](../support)

---
